...
size_t SerialDevice::readBuffer(uint8_t *values, size_t size, bool) {
  if (!values || !_stream) {
    return -1;
  }
  size_t readCounter = 0;
  while (_stream->available() > 0 && readCounter < size) {
...
size_t SerialDevice::writeBuffer(const uint8_t *values, size_t size, bool) {
  if (!values || !_stream) {
    return -1;
  }
